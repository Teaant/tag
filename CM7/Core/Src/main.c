...
extern volatile uint32_t serial_num;


uint32_t TIM15Count = 0;
volatile uint8_t is_imu_initialized = 0;
volatile uint8_t is_mag_calibrated = 0;
volatile uint8_t new_mag_data_flag = 0;
 ...
 //  is_imu_initialized = imuInit();
 	...
 	for(int i=0;i<DWT_NUM_DW_DEV;i++)
 	{
 		if(uwb_node.device->ports[i].avalible == 1)
 		{
 			...
 			//do not force the device into rx state immediately， however, when in needed to do so
// 			dwt_rxenable(DWT_START_RX_IMMEDIATE, &UWB.ports[i]);
 			HAL_NVIC_EnableIRQ(uwb_node.device->ports[i].exti_line);
 		}
 	}

 	HAL_GPIO_WritePin(GPIOE, GPIO_PIN_9, GPIO_PIN_SET);  //This is to do what
