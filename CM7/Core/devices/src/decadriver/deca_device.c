...
int dwt_writetxdata(uint16 txFrameLength, uint8 *txFrameBytes, uint16 txBufferOffset, UWBPortTypeDef *pports)
{
    ...
    if ((txBufferOffset + txFrameLength) <= 1024)
    {
        ...
        dwt_writetodevice( TX_BUFFER_ID, txBufferOffset, txFrameLength-2, txFrameBytes, pports);
        return DWT_SUCCESS;
    }
    else
    {
...
void dwt_isr(UWBPortTypeDef *pports)
{
    ...
    {
        dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_ALL_TX, pports); // Clear TX event bits
...
{
	uint64_t temp={0};
	dwt_readfromdevice(RX_TTCKO_ID, 0, RX_TTCKO_LEN, &temp, pports);
